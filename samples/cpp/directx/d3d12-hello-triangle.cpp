...
public:
	...
	virtual void OnDestroy() {
		// Ensure that the GPU is no longer referencing resources that are about
...
private:
	...
	void LoadPipeline() {
		...
		ThrowIfFailed(factory->MakeWindowAssociation(
			Win32Application::GetHwnd(), DXGI_MWA_NO_ALT_ENTER));
		...
		// Create frame resources.
		{
			...
			for (UINT n = 0; n < FrameCount; n++) {
				...
				m_device->CreateRenderTargetView(m_renderTargets[n].Get(),
												 nullptr, rtvHandle);
	...
	void LoadAssets() {
		// Create an empty root signature.
		{
			...
			ThrowIfFailed(D3D12SerializeRootSignature(
				&rootSignatureDesc, D3D_ROOT_SIGNATURE_VERSION_1, &signature,
				&error));
	...
	void PopulateCommandList() {
		...
		auto transitionTargetBarrier = CD3DX12_RESOURCE_BARRIER::Transition(
		...
		auto transitionPresentBarrier = CD3DX12_RESOURCE_BARRIER::Transition(
