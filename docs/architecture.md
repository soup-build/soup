...
### Root Recipe
When processing the state for a given Recipe file the system will walk up the directory structure for the first instance of a [Root Recipe](architecture/root-recipe.md). This file is used as a shared set of properties that can be used by multiple packages. For example, it can specify a shared output folder so all build artifacts can be stored in a single location outside the source folders.
...
Most interactions within the Generate phase will use the `SoupTask` definition. This task can edit the shared state (to influence downstream builds) and generate build operations that are allowed to edit the artifact store. If the system needs to analyze the contents of the files on disk to decide how to perform generate the build operations then it will contain one or more `SoupPreprocessorTask` definitions. If any of these tasks exist then the Generate phase is broken down into sub phases. It will first run all of the preprocessor tasks to generate preprocessor operations. The evaluate engine will then invoke these special operations that can only read file content and write the result state to the console in the form of SML. The second generate phase will then discover and run all of the `SoupTask` definitions, the same way it would without preprocessor tasks, with the only difference being the content of the preprocessor tasks will be available in the global state.
