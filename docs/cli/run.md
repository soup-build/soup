...
```
soup run <path> [-flavor <name>] [-- <arg1> <arg2> etc.]
```
...
`--` - An optional parameter signals that all of the following arguments are meant to be passed into the child executable.
...
```
soup run ./Code/MyProject/ -flavor Release -- MyParameter
```
