...
namespace PrintResults {
	...
	void PrintResults(Soup::Core::OperationResults &results) {
		for (const auto &[key, operationResult] : results.GetResults()) {
			...
			auto evaluateTimeSystem =
				std::chrono::clock_cast<std::chrono::system_clock>(operationResult.EvaluateTime);
#else
			...
			std::cout << "\tObservedInput: " << ToString(operationResult.ObservedInput)
					  << std::endl;
	...
	export void LoadAndPrintOperationResults(const Opal::Path &operationResultsFile) {
