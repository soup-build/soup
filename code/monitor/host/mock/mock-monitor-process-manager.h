// <copyright file="mock-monitor-process-manager.h" company="Soup">
...
namespace Monitor {
	...
	public:
		...
		void RegisterExecuteResult(std::string command, std::string output) {
			_executeResults.emplace(std::move(command), std::move(output));
		}
		...
		void RegisterExecuteCallback(
			std::string command, std::function<void(ISystemAccessMonitor &)> monitor) {
			_executeMonitors.emplace(std::move(command), std::move(monitor));
		}
		...
		std::shared_ptr<Opal::System::IProcess> CreateMonitorProcess(
			...
			auto id = m_uniqueId++;
			message << "CreateMonitorProcess: " << id << " [" << workingDirectory.ToString() << "] "
					<< executable.ToString();
			...
			if (findOutput != _executeResults.end()) {
				return std::make_shared<Opal::System::MockProcess>(
					id, _requests, 0, findOutput->second, std::string());
			} else {
				return std::make_shared<Opal::System::MockProcess>(
					id, _requests, 0, std::string(), std::string());
			}
		}
	};
}
