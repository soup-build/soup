// <copyright file="linux-trace-event-listener.h" company="Soup">
...
namespace Monitor::Linux {
			...
			switch (registers.Command) {
				...
				default: {
					throw std::runtime_error(
						std::format("Unknown system call type {0}", registers.Command));
				}
			}
		}
	...
	private:
		...
		std::string ReadNullTerminatedStringValue(pid_t pid, long addr) {
			...
			while (len) {
				errno = 0;
				dissected_long_t u = {.val = ptrace(PTRACE_PEEKDATA, pid, addr, 0)};
