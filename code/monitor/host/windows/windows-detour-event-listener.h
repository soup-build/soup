// <copyright file="windows-detour-event-listener.h" company="Soup">
		...
		void HandleDetourMessage(Message &message, uint32_t &offset) {
			...
			switch (eventType) {
				...
				case DetourEventType::GetTempFileNameA: {
					...
					auto result = ReadUInt32Value(message, offset);
					m_monitor->OnGetTempFileNameA(
						pathName, prefixString, unique, tempFileName, result);
					break;
				}
				...
				case DetourEventType::CopyFileA: {
					...
					auto result = ReadBoolValue(message, offset);
					auto wasBlocked = ReadBoolValue(message, offset);
					m_monitor->OnCopyFileA(
						existingFileName, newFileName, failIfExists, result, wasBlocked);
					break;
				}
				...
				case DetourEventType::CopyFileTransactedA: {
					...
					auto result = ReadBoolValue(message, offset);
					auto wasBlocked = ReadBoolValue(message, offset);
					m_monitor->OnCopyFileTransactedA(
						existingFileName, newFileName, result, wasBlocked);
					break;
				}
				case DetourEventType::CopyFileTransactedW: {
					...
					auto result = ReadBoolValue(message, offset);
					auto wasBlocked = ReadBoolValue(message, offset);
					m_monitor->OnCopyFileTransactedW(
						existingFileName, newFileName, result, wasBlocked);
					break;
				}
				case DetourEventType::CreateDirectoryExA: {
					...
					auto result = ReadBoolValue(message, offset);
					auto wasBlocked = ReadBoolValue(message, offset);
					m_monitor->OnCreateDirectoryExA(
				...
				case DetourEventType::CreateDirectoryExW: {
					...
					auto result = ReadBoolValue(message, offset);
					auto wasBlocked = ReadBoolValue(message, offset);
					m_monitor->OnCreateDirectoryExW(
						templateDirectory, newDirectory, result, wasBlocked);
					break;
				}
				case DetourEventType::CreateDirectoryTransactedA: {
					...
					auto result = ReadBoolValue(message, offset);
					auto wasBlocked = ReadBoolValue(message, offset);
					m_monitor->OnCreateDirectoryTransactedA(
						templateDirectory, newDirectory, result, wasBlocked);
					break;
				}
				case DetourEventType::CreateDirectoryTransactedW: {
					...
					auto result = ReadBoolValue(message, offset);
					auto wasBlocked = ReadBoolValue(message, offset);
					m_monitor->OnCreateDirectoryTransactedW(
						templateDirectory, newDirectory, result, wasBlocked);
					break;
				}
				case DetourEventType::CreateFileTransactedA: {
					...
					auto result = ReadUInt64Value(message, offset);
					auto wasBlocked = ReadBoolValue(message, offset);
					m_monitor->OnCreateFileTransactedA(
				...
				case DetourEventType::CreateFileTransactedW: {
					...
					auto result = ReadUInt64Value(message, offset);
					auto wasBlocked = ReadBoolValue(message, offset);
					m_monitor->OnCreateFileTransactedW(
						fileName, desiredAccess, shareMode, result, wasBlocked);
					break;
				}
				...
				case DetourEventType::CreateSymbolicLinkA: {
					...
					auto flags = ReadUInt32Value(message, offset);
					auto result = ReadBoolValue(message, offset);
					m_monitor->OnCreateSymbolicLinkA(
				...
				case DetourEventType::CreateSymbolicLinkW: {
					...
					auto flags = ReadUInt32Value(message, offset);
					auto result = ReadBoolValue(message, offset);
					m_monitor->OnCreateSymbolicLinkW(
						symlinkFileName, targetFileName, flags, result);
					break;
				}
				case DetourEventType::CreateSymbolicLinkTransactedA: {
					...
					auto flags = ReadUInt32Value(message, offset);
					auto result = ReadBoolValue(message, offset);
					m_monitor->OnCreateSymbolicLinkTransactedA(
						symlinkFileName, targetFileName, flags, result);
					break;
				}
				case DetourEventType::CreateSymbolicLinkTransactedW: {
					...
					auto flags = ReadUInt32Value(message, offset);
					auto result = ReadBoolValue(message, offset);
					m_monitor->OnCreateSymbolicLinkTransactedW(
						symlinkFileName, targetFileName, flags, result);
					break;
				}
				...
				case DetourEventType::MoveFileTransactedA: {
					...
					auto result = ReadBoolValue(message, offset);
					auto wasBlocked = ReadBoolValue(message, offset);
					m_monitor->OnMoveFileTransactedA(
						existingFileName, newFileName, flags, result, wasBlocked);
					break;
				}
				case DetourEventType::MoveFileTransactedW: {
					...
					auto result = ReadBoolValue(message, offset);
					auto wasBlocked = ReadBoolValue(message, offset);
					m_monitor->OnMoveFileTransactedW(
						existingFileName, newFileName, flags, result, wasBlocked);
					break;
				}
				case DetourEventType::MoveFileWithProgressA: {
					...
					auto result = ReadBoolValue(message, offset);
					auto wasBlocked = ReadBoolValue(message, offset);
					m_monitor->OnMoveFileWithProgressA(
						existingFileName, newFileName, flags, result, wasBlocked);
					break;
				}
				case DetourEventType::MoveFileWithProgressW: {
					...
					auto result = ReadBoolValue(message, offset);
					auto wasBlocked = ReadBoolValue(message, offset);
					m_monitor->OnMoveFileWithProgressW(
						existingFileName, newFileName, flags, result, wasBlocked);
					break;
				}
				...
				case DetourEventType::SetFileAttributesTransactedA: {
					...
					auto result = ReadBoolValue(message, offset);
					auto wasBlocked = ReadBoolValue(message, offset);
					m_monitor->OnSetFileAttributesTransactedA(
						fileName, fileAttributes, result, wasBlocked);
					break;
				}
				case DetourEventType::SetFileAttributesTransactedW: {
					...
					auto result = ReadBoolValue(message, offset);
					auto wasBlocked = ReadBoolValue(message, offset);
					m_monitor->OnSetFileAttributesTransactedW(
						fileName, fileAttributes, result, wasBlocked);
					break;
				}
