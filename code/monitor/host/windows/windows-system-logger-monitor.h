#pragma once
#include "iwindows-system-monitor.h"
...
namespace Monitor::Windows {
	...
	public:
		...
		virtual void OnCreateDirectoryA(
		...
		virtual void OnDefineDosDeviceW(
			uint32_t /*flags*/,
		...
		virtual void OnDeleteVolumeMountPointW(
		...
		virtual void OnFindFirstChangeNotificationA(
		...
		virtual void OnFindFirstChangeNotificationW(
		...
		virtual void OnGetCompressedFileSizeA(
		...
		virtual void OnGetDiskFreeSpaceA(
		...
		virtual void OnGetDiskFreeSpaceExA(
		...
		virtual void OnGetFileAttributesExA(
			std::string_view fileName, bool /*result*/, bool wasBlocked) override final {
		...
		virtual void OnGetFullPathNameA(
			std::string_view fileName, uint32_t /*result*/) override final {
			m_stream << "GetFullPathNameA: " << fileName << std::endl;
		}

		virtual void OnGetFullPathNameW(
			std::wstring_view fileName,
		...
		virtual void OnGetLongPathNameW(
		...
		virtual void OnGetShortPathNameW(
			std::wstring_view /*longPath*/,
			std::wstring_view /*shortPath*/,
			uint32_t /*result*/) override final {
		...
		virtual void OnGetTempFileNameA(
			std::string_view /*pathName*/,
		...
		virtual void OnGetTempFileNameW(
			std::wstring_view pathName,
		...
		virtual void OnGetTempPathA(
			std::string_view /*buffer*/, uint32_t /*result*/) override final {
			m_stream << "GetTempPathA: " << std::endl;
		}

		virtual void OnGetTempPathW(
			std::wstring_view /*buffer*/, uint32_t /*result*/) override final {
		...
		virtual void OnQueryDosDeviceW(
		...
		virtual void OnRemoveDirectoryA(
			std::string_view pathName, bool /*result*/, bool wasBlocked) override final {
		...
		virtual void OnSetFileAttributesA(
			std::string_view fileName, bool /*result*/, bool wasBlocked) override final {
		...
		virtual void OnCreateProcessAsUserA(
		...
		// UndocumentedApi
		virtual void OnPrivCopyFileExA(
			std::string_view existingFileName,
			std::string_view newFileName,
			bool /*result*/,
			bool wasBlocked) override final {
			m_stream << "PrivCopyFileExA: " << existingFileName << " -> " << newFileName << " "
					 << wasBlocked << std::endl;
		}

		virtual void OnPrivCopyFileExW(
		...
		// WinBase
		virtual void OnCopyFileA(
			std::string_view existingFileName,
			std::string_view newFileName,
			bool /*failIfExists*/,
			bool /*result*/,
			bool wasBlocked) override final {
			m_stream << "CopyFileA: " << existingFileName << " -> " << newFileName << " "
					 << wasBlocked << std::endl;
		}

		virtual void OnCopyFileW(
			std::wstring_view existingFileName,
			std::wstring_view newFileName,
			bool /*failIfExists*/,
			bool /*result*/,
		...
		virtual void OnCopyFileExA(
			...
			bool /*result*/,
			bool wasBlocked) override final {
			m_stream << "CopyFileExA: " << existingFileName << " -> " << newFileName << " "
					 << wasBlocked << std::endl;
		}
		...
		virtual void OnCopyFileTransactedA(
			...
			bool /*result*/,
			bool wasBlocked) override final {
			m_stream << "CopyFileTransactedA: " << existingFileName << " -> " << newFileName << " "
					 << wasBlocked << std::endl;
		}
		...
		virtual void OnCreateDirectoryTransactedA(
			...
			bool /*result*/,
			bool wasBlocked) override final {
			m_stream << "CreateDirectoryTransactedA: " << newDirectory << " " << wasBlocked
					 << std::endl;
		}
		...
		virtual void OnCreateHardLinkA(
			std::string_view fileName,
			std::string_view existingFileName,
			bool /*result*/) override final {
		...
		virtual void OnCreateHardLinkW(
			std::wstring_view fileName,
			std::wstring_view existingFileName,
			bool /*result*/) override final {
			std::wstring_convert<std::codecvt_utf8<wchar_t>, wchar_t> converter;
		...
		virtual void OnCreateHardLinkTransactedA(
			...
			std::string_view existingFileName,
			bool /*result*/) override final {
			m_stream << "CreateHardLinkTransactedA: " << fileName << " -> " << existingFileName
					 << std::endl;
		}

		virtual void OnCreateHardLinkTransactedW(
			std::wstring_view fileName,
			std::wstring_view /*existingFileName*/,
			bool /*result*/) override final {
			std::wstring_convert<std::codecvt_utf8<wchar_t>, wchar_t> converter;
		...
		virtual void OnCreateSymbolicLinkA(
			std::string_view symlinkFileName,
			std::string_view targetFileName,
			uint32_t /*flags*/,
			bool /*result*/) override final {
			m_stream << "CreateSymbolicLinkA: " << symlinkFileName << " -> " << targetFileName
					 << std::endl;
		}

		virtual void OnCreateSymbolicLinkW(
			std::wstring_view symlinkFileName,
		...
		virtual void OnCreateSymbolicLinkTransactedA(
			...
			uint32_t /*flags*/,
			bool /*result*/) override final {
			m_stream << "CreateSymbolicLinkTransactedA: " << symlinkFileName << " -> "
					 << targetFileName << std::endl;
		}

		virtual void OnCreateSymbolicLinkTransactedW(
			std::wstring_view symlinkFileName,
			std::wstring_view targetFileName,
			uint32_t /*flags*/,
			bool /*result*/) override final {
		...
		virtual void OnFileEncryptionStatusA(
			std::string_view fileName, bool /*result*/) override final {
		...
		virtual void OnGetCompressedFileSizeTransactedA(
			std::string_view fileName, uint32_t /*result*/) override final {
		...
		virtual void OnGetEnvironmentVariableA(
		...
		virtual void OnGetFileAttributesTransactedA(
			std::string_view fileName, uint32_t /*result*/, bool wasBlocked) override final {
			m_stream << "GetFileAttributesTransactedA: " << fileName << " " << wasBlocked
					 << std::endl;
		}
		...
		virtual void OnGetFileSecurityA(
			std::string_view fileName, bool /*result*/, bool wasBlocked) override final {
			m_stream << "GetFileSecurityA: " << fileName << " " << wasBlocked << std::endl;
		}

		virtual void OnGetFullPathNameTransactedA(
			std::string_view fileName, uint32_t /*result*/) override final {
		...
		virtual void OnGetLongPathNameTransactedA(
			std::string_view /*shortPath*/,
			std::string_view /*longPath*/,
			uint32_t /*result*/) override final {
		...
		virtual void OnGetLongPathNameTransactedW(
			std::wstring_view /*shortPath*/,
			std::wstring_view /*longPath*/,
			uint32_t /*result*/) override final {
		...
		virtual void OnGetShortPathNameA(
			std::string_view /*longPath*/,
			std::string_view /*shortPath*/,
			uint32_t /*result*/) override final {
		...
		virtual void OnMoveFileA(
			std::string_view existingFileName,
			std::string_view newFileName,
			bool /*result*/,
			bool wasBlocked) override final {
			m_stream << "MoveFileA: " << existingFileName << " -> " << newFileName << " "
					 << wasBlocked << std::endl;
		}

		virtual void OnMoveFileW(
			std::wstring_view existingFileName,
			std::wstring_view newFileName,
			bool /*result*/,
			bool wasBlocked) override final {
		...
		virtual void OnMoveFileExA(
			...
			bool /*result*/,
			bool wasBlocked) override final {
			m_stream << "MoveFileExA: " << existingFileName << " -> " << newFileName << " "
					 << wasBlocked << std::endl;
		}

		virtual void OnMoveFileExW(
			std::wstring_view existingFileName,
			std::wstring_view newFileName,
			uint32_t /*flags*/,
			bool /*result*/,
		...
		virtual void OnMoveFileTransactedA(
			...
			bool /*result*/,
			bool wasBlocked) override final {
			m_stream << "MoveFileTransactedA: " << existingFileName << " -> " << newFileName << " "
					 << wasBlocked << std::endl;
		}

		virtual void OnMoveFileTransactedW(
			std::wstring_view existingFileName,
			std::wstring_view newFileName,
			uint32_t /*flags*/,
			bool /*result*/,
		...
		virtual void OnMoveFileWithProgressA(
			std::string_view existingFileName,
			std::string_view newFileName,
			uint32_t /*flags*/,
			bool /*result*/,
			bool wasBlocked) override final {
			m_stream << "MoveFileWithProgressA: " << existingFileName << " -> " << newFileName
					 << " " << wasBlocked << std::endl;
		}

		virtual void OnMoveFileWithProgressW(
			std::wstring_view existingFileName,
			std::wstring_view newFileName,
			uint32_t /*flags*/,
			bool /*result*/,
		...
		virtual void OnOpenEncryptedFileRawA(
			std::string_view fileName, uint32_t /*flags*/, uint32_t /*result*/) override final {
		...
		virtual void OnRemoveDirectoryTransactedA(
			std::string_view pathName, bool /*result*/, bool wasBlocked) override final {
			m_stream << "RemoveDirectoryTransactedA: " << pathName << " " << wasBlocked
					 << std::endl;
		}
		...
		virtual void OnReplaceFileA(
			...
			bool /*result*/) override final {
			m_stream << "ReplaceFileA: " << replacedFileName << " - " << replacementFileName
					 << " - " << backupFileName << std::endl;
		}

		virtual void OnReplaceFileW(
		...
		virtual void OnSetCurrentDirectoryA(
			std::string_view pathName, bool /*result*/) override final {
		...
		virtual void OnSetFileAttributesTransactedA(
			...
			bool /*result*/,
			bool wasBlocked) override final {
			m_stream << "SetFileAttributesTransactedA: " << pathName << " " << wasBlocked
					 << std::endl;
		}

		virtual void OnSetFileAttributesTransactedW(
			std::wstring_view pathName,
			uint32_t /*fileAttributes*/,
			bool /*result*/,
			bool wasBlocked) override final {
		...
		virtual void OnSetFileShortNameA(
			std::string_view shortName, bool /*result*/) override final {
