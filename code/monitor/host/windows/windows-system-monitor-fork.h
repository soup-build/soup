...
namespace Monitor::Windows {
	...
	public:
		...
		void OnCreateFile2(
			...
			_monitor1->OnCreateFile2(
				fileName, desiredAccess, sharedMode, creationDisposition, result, wasBlocked);
			_monitor2->OnCreateFile2(
				fileName, desiredAccess, sharedMode, creationDisposition, result, wasBlocked);
		}
		...
		void OnFindFirstChangeNotificationA(
			std::string_view pathName, bool watchSubtree, uint32_t notifyFilter) override final {
		...
		void OnFindFirstChangeNotificationW(
			std::wstring_view pathName, bool watchSubtree, uint32_t notifyFilter) override final {
		...
		void OnGetFullPathNameW(
			std::wstring_view fileName, std::wstring_view buffer, uint32_t result) override final {
		...
		void OnGetLongPathNameA(
			std::string_view shortPath, std::string_view longPath, uint32_t result) override final {
		...
		// ProcessThreadsApi
		void OnCreateProcessA(
			bool wasDetoured, std::string_view applicationName, bool result) override final {
		...
		void OnCreateProcessW(
			bool wasDetoured, std::wstring_view applicationName, bool result) override final {
		...
		void OnCreateDirectoryTransactedA(
			...
			bool result,
			bool wasBlocked) override final {
			_monitor1->OnCreateDirectoryTransactedA(
				templateDirectory, newDirectory, result, wasBlocked);
			_monitor2->OnCreateDirectoryTransactedA(
				templateDirectory, newDirectory, result, wasBlocked);
		}
		...
		void OnCreateDirectoryTransactedW(
			...
			bool result,
			bool wasBlocked) override final {
			_monitor1->OnCreateDirectoryTransactedW(
				templateDirectory, newDirectory, result, wasBlocked);
			_monitor2->OnCreateDirectoryTransactedW(
				templateDirectory, newDirectory, result, wasBlocked);
		}
		...
		void OnCreateFileTransactedA(
			...
			uint64_t result,
			bool wasBlocked) override final {
			_monitor1->OnCreateFileTransactedA(
				fileName, desiredAccess, shareMode, result, wasBlocked);
			_monitor2->OnCreateFileTransactedA(
				fileName, desiredAccess, shareMode, result, wasBlocked);
		}
		...
		void OnCreateFileTransactedW(
			...
			uint64_t result,
			bool wasBlocked) override final {
			_monitor1->OnCreateFileTransactedW(
				fileName, desiredAccess, shareMode, result, wasBlocked);
			_monitor2->OnCreateFileTransactedW(
				fileName, desiredAccess, shareMode, result, wasBlocked);
		}

		void OnCreateHardLinkA(
			std::string_view fileName,
			std::string_view existingFileName,
			bool result) override final {
		...
		void OnCreateHardLinkTransactedA(
			std::string_view fileName,
			std::string_view existingFileName,
			bool result) override final {
		...
		void OnCreateHardLinkTransactedW(
			std::wstring_view fileName,
			std::wstring_view existingFileName,
			bool result) override final {
		...
		void OnCreateProcessWithLogonW(
		...
		void OnCreateSymbolicLinkTransactedA(
			std::string_view symlinkFileName,
			std::string_view targetFileName,
			uint32_t flags,
			bool result) override final {
			_monitor1->OnCreateSymbolicLinkTransactedA(
				symlinkFileName, targetFileName, flags, result);
			_monitor2->OnCreateSymbolicLinkTransactedA(
				symlinkFileName, targetFileName, flags, result);
		}

		void OnCreateSymbolicLinkTransactedW(
			std::wstring_view symlinkFileName,
			std::wstring_view targetFileName,
			uint32_t flags,
			bool result) override final {
			_monitor1->OnCreateSymbolicLinkTransactedW(
				symlinkFileName, targetFileName, flags, result);
			_monitor2->OnCreateSymbolicLinkTransactedW(
				symlinkFileName, targetFileName, flags, result);
		}
		...
		void OnFindFirstFileNameTransactedW(
		...
		void OnGetCompressedFileSizeTransactedW(
		...
		void OnGetShortPathNameA(
			std::string_view longPath, std::string_view shortPath, uint32_t result) override final {
		...
		void OnMoveFileTransactedA(
			...
			bool result,
			bool wasBlocked) override final {
			_monitor1->OnMoveFileTransactedA(
				existingFileName, newFileName, flags, result, wasBlocked);
			_monitor2->OnMoveFileTransactedA(
				existingFileName, newFileName, flags, result, wasBlocked);
		}
		...
		void OnMoveFileTransactedW(
			...
			bool result,
			bool wasBlocked) override final {
			_monitor1->OnMoveFileTransactedW(
				existingFileName, newFileName, flags, result, wasBlocked);
			_monitor2->OnMoveFileTransactedW(
				existingFileName, newFileName, flags, result, wasBlocked);
		}
		...
		void OnMoveFileWithProgressA(
			...
			bool result,
			bool wasBlocked) override final {
			_monitor1->OnMoveFileWithProgressA(
				existingFileName, newFileName, flags, result, wasBlocked);
			_monitor2->OnMoveFileWithProgressA(
				existingFileName, newFileName, flags, result, wasBlocked);
		}
		...
		void OnMoveFileWithProgressW(
			...
			bool result,
			bool wasBlocked) override final {
			_monitor1->OnMoveFileWithProgressW(
				existingFileName, newFileName, flags, result, wasBlocked);
			_monitor2->OnMoveFileWithProgressW(
				existingFileName, newFileName, flags, result, wasBlocked);
		}

		void OnOpenEncryptedFileRawA(
		...
		void OnRemoveDirectoryTransactedW(
			std::wstring_view pathName, bool result, bool wasBlocked) override final {
		...
		void OnReplaceFileA(
			...
			bool result) override final {
			_monitor1->OnReplaceFileA(
				replacedFileName, replacementFileName, backupFileName, replaceFlags, result);
			_monitor2->OnReplaceFileA(
				replacedFileName, replacementFileName, backupFileName, replaceFlags, result);
		}
		...
		void OnReplaceFileW(
			...
			uint32_t replaceFlags,
			bool result) override final {
			_monitor1->OnReplaceFileW(
				replacedFileName, replacementFileName, backupFileName, replaceFlags, result);
			_monitor2->OnReplaceFileW(
				replacedFileName, replacementFileName, backupFileName, replaceFlags, result);
		}
		...
		void OnSetFileAttributesTransactedA(
			std::string_view pathName,
		...
		void OnSetFileAttributesTransactedW(
			std::wstring_view pathName,
			uint32_t fileAttributes,
			bool result,
			bool wasBlocked) override final {
