...
namespace Soup::View {
	ftxui::Component LayoutGeneratePhaseTasks(
		const Core::ValueTable &generatePhase1Info, int *selected, int *showGraphView) {
		...
		for (auto &taskNameValue : findRuntimeOrderResult->second.AsList()) {
			...
			tasksPropertiesComponents.push_back(ScrollFrame(TreeView(std::move(taskInfo))));
		}
		...
		auto tasksPropertiesView =
			ftxui::Container::Tab(std::move(tasksPropertiesComponents), selected);
