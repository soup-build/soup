...
namespace Soup::View {
	...
	export PackageLoadState LoadPackage(
		...
		auto generatePhase1ResultFile =
			soupTargetDirectory + Core::Build::Constants::GeneratePhase1ResultFileName();
		auto generatePhase1Result = Core::GenerateResult();
		bool generatePhase1IsPreprocessor = false;
		if (Core::GenerateResultManager::TryLoadState(
		...
		auto evaluatePhase1ResultsFile =
			soupTargetDirectory + Core::Build::Constants::EvaluatePhase1ResultsFileName();
		auto evaluatePhase1Results = Core::OperationResults();
		if (Core::OperationResultsManager::TryLoadState(
				evaluatePhase1ResultsFile, evaluatePhase1Results, fileSystemState)) {
			result.EvaluatePhase1Results = std::move(evaluatePhase1Results);
		}

		auto generatePhase1InfoFile =
			soupTargetDirectory + Core::Build::Constants::GeneratePhase1InfoFileName();
		...
		if (generatePhase1IsPreprocessor) {
			auto generatePhase2ResultFile =
				soupTargetDirectory + Core::Build::Constants::GeneratePhase2ResultFileName();
			...
			auto evaluatePhase2ResultsFile =
				soupTargetDirectory + Core::Build::Constants::EvaluatePhase2ResultsFileName();
			auto evaluatePhase2Results = Core::OperationResults();
			if (Core::OperationResultsManager::TryLoadState(
					evaluatePhase2ResultsFile, evaluatePhase2Results, fileSystemState)) {
				result.EvaluatePhase2Results = std::move(evaluatePhase2Results);
			}

			auto generatePhase2InfoFile =
				soupTargetDirectory + Core::Build::Constants::GeneratePhase2InfoFileName();
