// <copyright file="argument-parser.h" company="Soup">
...
class ArgumentsParser {
  public:
	static ArgumentsParser Parse(const std::vector<std::string> &args) {
		...
		if (commandType == "build") {
			...
			if (TryGetValueArgument("architecture", unusedArgs,
									architectureValue)) {
			...
			if (TryGetValueArgument("p", unusedArgs, parallelizationValue)) {
				...
				if (options->Parallelization > 20)
					throw std::runtime_error(
						"Invalid amount of parallelization");
			}
			...
			throw std::runtime_error(
				std::format("Unknown command argument: {}", commandType));
		}
		...
		return ArgumentsParser(std::move(result));
	}
  ...
  public:
	ArgumentsParser() : _result(nullptr) {}
	...
	template <typename T> auto ExtractResult() -> T {
		return *dynamic_cast<T *>(_result.get());
	}

  private:
	static bool TryGetIndexArgument(std::vector<std::string> &unusedArgs,
									std::string &argument) {
		// Check if the first argument is not a dash flag
		if (!unusedArgs.empty() && !unusedArgs[0].empty() &&
			unusedArgs[0][0] != '-') {
	...
	static bool IsFlagSet(const char *name,
						  std::vector<std::string> &unusedArgs) {
		auto flagValue = std::string("-") + name;
		Log::Diag("IsFlagSet: {}", flagValue);
		auto flagLocation =
			std::find(unusedArgs.begin(), unusedArgs.end(), flagValue);
	...
	static void SplitArguments(const char *name,
							   std::vector<std::string> &unusedArgs,
		...
		if (flagLocation != unusedArgs.end()) {
			...
			std::move(argsStart, unusedArgs.end(),
					  std::back_inserter(splitArgs));
			unusedArgs.erase(flagLocation, unusedArgs.end());
		}
	}

	static bool TryGetValueArgument(const char *name,
									std::vector<std::string> &unusedArgs,
									std::string &value) {
		auto nameValue = std::string("-") + name;
		Log::Diag("TryGetValueArgument: {}", nameValue);
		auto nameLocation =
			std::find(unusedArgs.begin(), unusedArgs.end(), nameValue);
	...
	static TraceEventFlag CheckVerbosity(std::vector<std::string> &unusedArgs) {
		auto level = static_cast<uint32_t>(TraceEventFlag::HighPriority) |
