...
export namespace Soup::Core {
	...
	public:
		static void SetSlotTable(WrenVM *vm, int slot,
		...
		static void SetSlotValue(WrenVM *vm, int slot, const Value &value) {
			...
			case ValueType::Version:
				wrenSetSlotString(vm, slot,
								  value.AsVersion().ToString().c_str());
				break;
			case ValueType::PackageReference:
				wrenSetSlotString(
					vm, slot, value.AsPackageReference().ToString().c_str());
				break;
			case ValueType::LanguageReference:
				wrenSetSlotString(
					vm, slot, value.AsLanguageReference().ToString().c_str());
				break;
			default:
