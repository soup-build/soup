// <copyright file="recipe-cache.cpp" company="Soup">
...
namespace Soup::Core {
	/// <summary>
	/// The recipe cache that maintains an in memory collection of recipes to
	/// prevent loading multiple instances from disk
	/// </summary>
	...
	public:
		...
		bool TryGetOrLoadRecipe(const Path &recipeFile, const Recipe *&result) {
			// Check if the recipe was already loaded
			auto findRecipe = _knownRecipes.find(recipeFile.ToString());
			if (findRecipe != _knownRecipes.end()) {
				result = &findRecipe->second;
