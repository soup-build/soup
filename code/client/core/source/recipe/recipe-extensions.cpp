// <copyright file="recipe-extensions.cpp" company="Soup">
...
namespace Soup::Core {
	...
	public:
		...
		/// </summary>
		static bool TryLoadRecipeFromFile(const Path &recipeFile, Recipe &result) {
			...
			try {
				result = Recipe(RecipeSML::Deserialize(recipeFile, file->GetInStream()));
				return true;
		...
		static Path GetPackageRecipeFile(
			const Path &workingDirectory, const PackageReference &package) {
			if (package.IsLocal()) {
