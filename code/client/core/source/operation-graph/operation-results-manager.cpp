// <copyright file="operation-results-manager.cpp" company="Soup">
...
namespace Soup::Core {
	...
	public:
		...
		/// </summary>
		static bool TryLoadState(const Path &operationResultsFile,
								 OperationResults &result,
								 FileSystemState &fileSystemState) {
			// Open the file to read from
			std::shared_ptr<System::IInputFile> file;
			if (!System::IFileSystem::Current().TryOpenRead(
					operationResultsFile, true, file)) {
			...
			try {
				result = OperationResultsReader::Deserialize(
					file->GetInStream(), fileSystemState);
				return true;
		...
		static void SaveState(const Path &operationResultsFile,
			...
			for (auto &resultReference : state.GetResults()) {
				...
				files.insert(result.ObservedInput.begin(),
							 result.ObservedInput.end());
				files.insert(result.ObservedOutput.begin(),
							 result.ObservedOutput.end());
			}

			// Write the build state to the file stream
			OperationResultsWriter::Serialize(state, files, fileSystemState,
											  file->GetOutStream());
		}
	};
}
