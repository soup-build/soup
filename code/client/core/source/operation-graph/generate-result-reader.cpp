// <copyright file="generate-result-reader.cpp" company="Soup">
...
namespace Soup::Core {
	...
	public:
		static GenerateResult Deserialize(std::istream &stream,
			...
			if (offset != contentBuffer.size()) {
				throw std::runtime_error("Generate Result file corrupted - Did "
	...
	private:
		static GenerateResult Deserialize(char *data, size_t size,
										  size_t &offset,
										  FileSystemState &fileSystemState) {
			...
			if (headerBuffer[0] != 'B' || headerBuffer[1] != 'G' ||
			...
			if (fileVersion != FileVersion) {
				throw std::runtime_error(
					"Generate Result file version does not match expected");
			}
			...
			if (headerBuffer[0] != 'F' || headerBuffer[1] != 'I' ||
				...
				throw std::runtime_error(
					"Invalid generate result files header");
			}
			...
			Read(data, size, offset, headerBuffer.data(), 4);
			if (headerBuffer[0] != 'R' || headerBuffer[1] != 'O' ||
				headerBuffer[2] != 'P' || headerBuffer[3] != '\0') {
				throw std::runtime_error(
					"Invalid generate result root operations header");
			}
			...
			// Read the set of operations
			Read(data, size, offset, headerBuffer.data(), 4);
			if (headerBuffer[0] != 'O' || headerBuffer[1] != 'P' ||
				headerBuffer[2] != 'S' || headerBuffer[3] != '\0') {
				throw std::runtime_error(
					"Invalid generate result operations header");
			}
			...
			for (auto i = 0u; i < operationCount; i++) {
				operations[i] = OperationGraphReader::ReadOperationInfo(
					data, size, offset, activeFileIdMap);
			}

			return GenerateResult(OperationGraph(std::move(rootOperationIds),
												 std::move(operations)),
								  isPreprocessor);
		}
		...
		static std::vector<std::string> ReadStringList(char *data, size_t size,
													   size_t &offset) {
