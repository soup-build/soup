...
namespace Soup::Core {
		...
		static GenerateResult Deserialize(std::istream &stream, FileSystemState &fileSystemState) {
			...
			if (offset != contentBuffer.size()) {
				throw std::runtime_error(
					"Generate Result file corrupted - Did "
	...
	private:
		static GenerateResult Deserialize(
			...
			if (headerBuffer[0] != 'B' || headerBuffer[1] != 'G' || headerBuffer[2] != 'R' ||
				headerBuffer[3] != '\0') {
			...
			Read(data, size, offset, headerBuffer.data(), 4);
			if (headerBuffer[0] != 'R' || headerBuffer[1] != 'O' || headerBuffer[2] != 'P' ||
				headerBuffer[3] != '\0') {
			...
			// Read the set of operations
			Read(data, size, offset, headerBuffer.data(), 4);
			if (headerBuffer[0] != 'O' || headerBuffer[1] != 'P' || headerBuffer[2] != 'S' ||
				headerBuffer[3] != '\0') {
			...
			for (auto i = 0u; i < operationCount; i++) {
				operations[i] =
					OperationGraphReader::ReadOperationInfo(data, size, offset, activeFileIdMap);
			}

			return GenerateResult(
				OperationGraph(std::move(rootOperationIds), std::move(operations)), isPreprocessor);
		}
