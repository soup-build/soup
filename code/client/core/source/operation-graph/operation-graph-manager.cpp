// <copyright file="operation-graph-manager.cpp" company="Soup">
...
namespace Soup::Core {
	...
	public:
		...
		/// </summary>
		static bool TryLoadState(const Path &operationGraphFile,
								 OperationGraph &result,
								 FileSystemState &fileSystemState) {
			// Open the file to read from
			std::shared_ptr<System::IInputFile> file;
			if (!System::IFileSystem::Current().TryOpenRead(operationGraphFile,
															true, file)) {
			...
			try {
				result = OperationGraphReader::Deserialize(file->GetInStream(),
														   fileSystemState);
				return true;
		...
		/// </summary>
		static void SaveState(const Path &operationGraphFile,
			...
			for (auto &operationReference : state.GetOperations()) {
				...
				files.insert(operation.DeclaredInput.begin(),
							 operation.DeclaredInput.end());
				files.insert(operation.DeclaredOutput.begin(),
							 operation.DeclaredOutput.end());
				files.insert(operation.ReadAccess.begin(),
							 operation.ReadAccess.end());
				files.insert(operation.WriteAccess.begin(),
							 operation.WriteAccess.end());
			}

			// Open the file to write to
			auto file = System::IFileSystem::Current().OpenWrite(
				operationGraphFile, true);

			// Write the build state to the file stream
			OperationGraphWriter::Serialize(state, files, fileSystemState,
											file->GetOutStream());
		}
	};
}
