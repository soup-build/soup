// <copyright file="generate-result-manager.cpp" company="Soup">
...
namespace Soup::Core {
	...
	public:
		...
		/// </summary>
		static bool TryLoadState(const Path &generateResultFile,
								 GenerateResult &result,
								 FileSystemState &fileSystemState) {
			// Open the file to read from
			std::shared_ptr<System::IInputFile> file;
			if (!System::IFileSystem::Current().TryOpenRead(generateResultFile,
															true, file)) {
			...
			try {
				result = GenerateResultReader::Deserialize(file->GetInStream(),
														   fileSystemState);
				return true;
		...
		/// </summary>
		static void SaveState(const Path &generateResultFile,
			...
			GenerateResultWriter::Serialize(state, files, fileSystemState,
											file->GetOutStream());
		}
	};
}
