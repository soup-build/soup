...
namespace Soup::Core {
	...
	public:
		static OperationGraph Deserialize(std::istream &stream, FileSystemState &fileSystemState) {
			...
			if (offset != contentBuffer.size()) {
				throw std::runtime_error(
					"Operation Graph file corrupted - Did "
		...
		static OperationInfo ReadOperationInfo(
			...
			return OperationInfo(
				id,
				std::move(title),
				CommandInfo(Path(workingDirectory), Path(executable), std::move(arguments)),
	...
	private:
		static OperationGraph Deserialize(
			...
			if (headerBuffer[0] != 'B' || headerBuffer[1] != 'O' || headerBuffer[2] != 'G' ||
				headerBuffer[3] != '\0') {
			...
			return OperationGraph(std::move(rootOperationIds), std::move(operations));
		}
