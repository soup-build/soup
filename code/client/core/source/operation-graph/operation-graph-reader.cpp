// <copyright file="operation-graph-reader.cpp" company="Soup">
	...
	public:
		static OperationGraph Deserialize(std::istream &stream,
			...
			if (offset != contentBuffer.size()) {
				throw std::runtime_error("Operation Graph file corrupted - Did "
		...
		static OperationInfo ReadOperationInfo(
			...
			return OperationInfo(
				...
				CommandInfo(Path(workingDirectory), Path(executable),
							std::move(arguments)),
				std::move(declaredInput), std::move(declaredOutput),
				std::move(readAccess), std::move(writeAccess),
				std::move(children), dependencyCount);
		}

	private:
		static OperationGraph Deserialize(char *data, size_t size,
										  size_t &offset,
										  FileSystemState &fileSystemState) {
			...
			if (headerBuffer[0] != 'B' || headerBuffer[1] != 'O' ||
				headerBuffer[2] != 'G' || headerBuffer[3] != '\0') {
			...
			if (headerBuffer[0] != 'R' || headerBuffer[1] != 'O' ||
				...
				throw std::runtime_error(
					"Invalid operation graph root operations header");
			}
			...
			for (auto i = 0u; i < operationCount; i++) {
				operations[i] =
					ReadOperationInfo(data, size, offset, activeFileIdMap);
			}

			return OperationGraph(std::move(rootOperationIds),
								  std::move(operations));
		}
