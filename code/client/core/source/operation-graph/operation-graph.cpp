// <copyright file="operation-graph.cpp" company="Soup">
...
namespace Soup::Core {
	/// <summary>
	/// The operation graph that represents the set of operations that need to
	/// be evaluated to perform the build
	/// </summary>
	...
	public:
		...
		OperationGraph(
			...
			: _rootOperations(std::move(rootOperations)),
			  _operations(),
			  _operationLookup() {
			// Store the incoming vector of operations as a lookup for fast
			// checks
