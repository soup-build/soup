// <copyright file="system-access-tracker.cpp" company="Soup">
	...
	public:
		...
		void VerifyResult() {
			if (_activeProcessCount != 0) {
				Log::Warning(
					"A child process is still running in the background");
			}
		...
		std::set<std::string> GetInput() { return _input; }

		std::set<std::string> GetOutput() { return _output; }

		virtual void OnCreateProcess(std::string_view applicationName,
									 bool wasDetoured) override final {
			if (wasDetoured)
				Log::Diag("SystemAccessTracker::OnCreateDetouredProcess - {}",
						  applicationName);
			else
				Log::Diag("SystemAccessTracker::OnCreateProcess - {}",
						  applicationName);
		}

		virtual void TouchFileRead(Path filePath, bool exists,
		...
		virtual void TouchFileWrite(Path filePath,
									bool wasBlocked) override final {
			if (wasBlocked) {
				// TODO: Warning
		...
		virtual void TouchFileDelete(Path filePath,
									 bool wasBlocked) override final {
			if (wasBlocked) {
				// TODO: Warning
				...
				// If this was an output file extract it as it was a transient
				// file
		...
		virtual void SearchPath(std::string_view path,
								std::string_view filename) override final {
