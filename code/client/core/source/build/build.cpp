...
namespace Soup::Core::Build {
	...
	export std::map<std::string, KnownLanguage> GetKnownLanguages() {
		auto result = std::map<std::string, KnownLanguage>({
			{"C", KnownLanguage("soup", "c")},
			{"C++", KnownLanguage("soup", "cpp")},
			{"C#", KnownLanguage("soup", "csharp")},
			{"Wren", KnownLanguage("soup", "wren")},
		});
	...
	void PreloadFileSystemState(PackageProvider &packageProvider,
		...
		for (auto package : packageProvider.GetPackageLookup()) {
			...
			// TODO:
			// fileSystemState.PreloadDirectory(package.second.TargetDirectory,
			// false);
		}
