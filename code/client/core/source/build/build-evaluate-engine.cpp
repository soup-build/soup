...
namespace Soup::Core {
			...
			if (!rootOperations.empty()) {
				...
				for (auto i = 1u; i <= _threadCount; i++) {
					workerThreads.push_back(
						std::thread(
							&BuildEvaluateEngine::WorkerThread, this, std::ref(evaluateState), i));
				}
	...
	private:
		...
		void CheckExecuteOperation(
			BuildEvaluateGraphState &evaluateState, const CurrentOperationState operationState) {
			...
			if (operationState.PreviousResult && operationState.PreviousResult->WasSuccessfulRun) {
				...
				if (operationState.Info.Command.Executable != Path("./writefile.exe")) {
					...
					if (_stateChecker.IsOutdated(
							operationState.PreviousResult->EvaluateTime, executableFileId)) {
			...
			if (buildRequired) {
				...
				if (operationState.Info.Command.Executable == Path("./writefile.exe")) {
					ExecuteWriteFileOperation(operationState.Info, operationResult);
				} else {
					ExecuteOperation(
				...
				evaluateState.UpdateOperationResult(
					operationState.Info.Id, std::move(operationResult));
			} else {
		...
		void ExecuteWriteFileOperation(
			const OperationInfo &operationInfo, OperationResult &operationResult) {
			...
			auto filePath =
				fileName.HasRoot() ? fileName : operationInfo.Command.WorkingDirectory + fileName;
		...
		void ExecuteOperation(
			...
			if (exitCode == 0) {
				...
				operationResult.ObservedInput =
					_fileSystemState.ToFileIds(input, operationInfo.Command.WorkingDirectory);
				operationResult.ObservedOutput =
					_fileSystemState.ToFileIds(output, operationInfo.Command.WorkingDirectory);
		...
		void VerifyObservedState(
				...
				if (evaluateState.TryGetOutputFileOperation(fileId, matchedOutputOperationId) &&
					...
					if (!evaluateState.TryGetInputFileOperations(
						...
						auto &existingOperation =
							evaluateState.Graph.GetOperationInfo(matchedOutputOperationId);
			...
			for (auto fileId : operationResult.ObservedOutput) {
				...
				auto findObservedInput = std::find(
					operationResult.ObservedInput.begin(),
					operationResult.ObservedInput.end(),
					fileId);
				if (findObservedInput != operationResult.ObservedInput.end()) {
					auto filePath = _fileSystemState.GetFilePath(fileId);
					Log::Warning(
						"File \"{}\" observed as both input and "
						"output for operation \"{}\"",
						filePath.ToString(),
						operationInfo.Title);
					Log::Warning(
						"Removing from input list for now. Will be "
