...
namespace Soup::Core {
	/// <summary>
	/// The core build evaluation engine that knows how to perform a build from
	/// a provided Operation Graph.
	/// </summary>
	...
	public:
		/// <summary>
		/// Initializes a new instance of the <see cref="BuildEvaluateEngine"/>
		/// class.
		/// </summary>
		BuildEvaluateEngine(unsigned int threadCount, bool forceRebuild,
							bool disableMonitor, bool partialMonitor,
		...
		/// evaluate engine
		/// </summary>
		bool Evaluate(OperationGraph &operationGraph,
			...
			if (!rootOperations.empty()) {
				...
				auto evaluateState = BuildEvaluateGraphState(
					...
					globalAllowedReadAccess, globalAllowedWriteAccess,
					processStdOut);
				...
				for (auto i = 1u; i <= _threadCount; i++) {
					workerThreads.push_back(
						std::thread(&BuildEvaluateEngine::WorkerThread, this,
									std::ref(evaluateState), i));
				}
	...
	private:
		void WorkerThread(BuildEvaluateGraphState &evaluateState, size_t id) {
			...
			while (operationState.has_value()) {
				try {
					...
					CheckExecuteOperation(evaluateState,
										  operationState.value());

					// Scan the children for those ready to be built
					evaluateState.RegisterReadyChildren(
						operationState.value().Info);

					// Pop the next operation available or wait for one to be
					// queued up
		...
		/// <summary>
		/// Check if an individual operation has been run and execute if
		/// required
		/// </summary>
		void CheckExecuteOperation(BuildEvaluateGraphState &evaluateState,
			...
			if (operationState.PreviousResult &&
				...
				if (operationState.Info.Command.Executable !=
					Path("./writefile.exe")) {
			...
			if (buildRequired) {
				...
				messageBuilder
					<< "Execute: ["
					<< operationState.Info.Command.WorkingDirectory.ToString()
					<< "] ";
				messageBuilder
					<< operationState.Info.Command.Executable.ToString();
				...
				if (operationState.Info.Command.Executable ==
					Path("./writefile.exe")) {
					ExecuteWriteFileOperation(operationState.Info,
											  operationResult);
				} else {
					ExecuteOperation(evaluateState.TemporaryDirectory,
									 evaluateState.GlobalAllowedReadAccess,
									 evaluateState.GlobalAllowedWriteAccess,
									 evaluateState.ProcessStdOut,
									 operationState.Info, operationResult);
				}

				// Ensure there are no new dependencies
				VerifyObservedState(evaluateState, operationState.Info,
									operationResult);

				evaluateState.UpdateOperationResult(operationState.Info.Id,
		...
		/// </summary>
		void ExecuteWriteFileOperation(const OperationInfo &operationInfo,
			...
			auto filePath =
				fileName.HasRoot()
					? fileName
					: operationInfo.Command.WorkingDirectory + fileName;
			auto &content = operationInfo.Command.Arguments[1];

			// Open the file to write to
			auto file =
				System::IFileSystem::Current().OpenWrite(filePath, false);
			file->GetOutStream() << content;

			operationResult.ObservedInput = {};
			operationResult.ObservedOutput = {
				_fileSystemState.ToFileId(
					filePath, operationInfo.Command.WorkingDirectory),
			};

			// Mark this operation as successful to enable future incremental
			// builds
			operationResult.WasSuccessfulRun = true;
			operationResult.EvaluateTime =
				System::ISystem::Current().GetCurrentTime();

			// Ensure the File System State is notified of any output files that
			// have changed
			_fileSystemState.InvalidateFileWriteTimes(
				operationResult.ObservedOutput);
		}
		...
		void ExecuteOperation(
			...
			if (enableAccessChecks) {
				allowedReadAccess =
					_fileSystemState.GetFilePaths(operationInfo.ReadAccess);
				allowedWriteAccess =
					_fileSystemState.GetFilePaths(operationInfo.WriteAccess);
			}

			// Allow access to the global overrides
			std::copy(globalAllowedReadAccess.begin(),
					  globalAllowedReadAccess.end(),
					  std::back_inserter(allowedReadAccess));
			std::copy(globalAllowedWriteAccess.begin(),
					  globalAllowedWriteAccess.end(),
					  std::back_inserter(allowedWriteAccess));
			...
			if (_disableMonitor) {
				process = System::IProcessManager::Current().CreateProcess(
					...
					operationInfo.Command.WorkingDirectory, true);
			} else {
				process = Monitor::IMonitorProcessManager::Current()
							  .CreateMonitorProcess(
								  operationInfo.Command.Executable,
								  operationInfo.Command.Arguments,
								  operationInfo.Command.WorkingDirectory,
								  environment, monitor, enableAccessChecks,
								  _partialMonitor, std::move(allowedReadAccess),
			...
			if (exitCode == 0) {
				...
				operationResult.ObservedInput = _fileSystemState.ToFileIds(
					input, operationInfo.Command.WorkingDirectory);
				operationResult.ObservedOutput = _fileSystemState.ToFileIds(
					output, operationInfo.Command.WorkingDirectory);

				// Mark this operation as successful to enable future
				// incremental builds
				operationResult.WasSuccessfulRun = true;
				operationResult.EvaluateTime =
					System::ISystem::Current().GetCurrentTime();
				...
				if (processStdOut.has_value()) {
					operationResult.ObservedValues =
						processStdOut.value()(stdOut);
				}

				// Ensure the File System State is notified of any output files
				// that have changed
				_fileSystemState.InvalidateFileWriteTimes(
					operationResult.ObservedOutput);
			} else {
				// Leave the previous state untouched and abandon the remaining
				// operations
				Log::Error("Operation exited with non-success code: {}",
						   exitCode);
				throw BuildFailedException();
			}
		}

		void VerifyObservedState(const BuildEvaluateGraphState &evaluateState,
								 const OperationInfo &operationInfo,
								 OperationResult &operationResult) {
			// TODO: Should generate NEW input/output lookup to check for
			// entirely observed dependencies
			...
			for (auto fileId : operationResult.ObservedInput) {
				...
				if (evaluateState.TryGetOutputFileOperation(
						fileId, matchedOutputOperationId) &&
					operationInfo.Id != matchedOutputOperationId) {
					// If it is a known output file then it must be a declared
					// input for this operation
					const std::set<OperationId> *matchedInputOperationIds;
					if (!evaluateState.TryGetInputFileOperations(
							fileId, matchedInputOperationIds) ||
						!matchedInputOperationIds->contains(operationInfo.Id)) {
						auto filePath = _fileSystemState.GetFilePath(fileId);
						auto &existingOperation =
							evaluateState.Graph.GetOperationInfo(
								matchedOutputOperationId);
						auto message = std::format(
							"File \"{}\" observed as input for operation "
							"\"{}\" was written to by operation \"{}\" and "
							"must be declared as input",
							filePath.ToString(), operationInfo.Title,
			...
			for (auto fileId : operationResult.ObservedOutput) {
				...
				auto findObservedInput =
					std::find(operationResult.ObservedInput.begin(),
							  operationResult.ObservedInput.end(), fileId);
				if (findObservedInput != operationResult.ObservedInput.end()) {
					auto filePath = _fileSystemState.GetFilePath(fileId);
					Log::Warning("File \"{}\" observed as both input and "
								 "output for operation \"{}\"",
								 filePath.ToString(), operationInfo.Title);
					Log::Warning("Removing from input list for now. Will be "
								 "treated as error in the future.");
				...
				if (evaluateState.TryGetOutputFileOperation(
					...
					if (matchedOutputOperationId != operationInfo.Id) {
						...
						auto &existingOperation =
							evaluateState.Graph.GetOperationInfo(
								matchedOutputOperationId);
						auto message = std::format(
							"File \"{}\" observed as output for operation "
							"\"{}\" was already written by operation \"{}\"",
							filePath.ToString(), operationInfo.Title,
							existingOperation.Title);
						throw std::runtime_error(message);
					}
					...
					if (evaluateState.TryGetInputFileOperations(
							fileId, matchedInputOperationIds)) {
						if (!matchedInputOperationIds->contains(
								operationInfo.Id)) {
							auto filePath =
								_fileSystemState.GetFilePath(fileId);
							auto message = std::format(
								"File \"{}\" observed as output from operation "
								"\"{}\" creates new dependency to existing "
								"declared inputs",
								filePath.ToString(), operationInfo.Title);
							throw std::runtime_error(message);
						}
