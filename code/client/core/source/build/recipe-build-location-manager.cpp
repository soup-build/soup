// <copyright file="recipe-build-location-manager.cpp" company="Soup">
...
namespace Soup::Core {
	...
	public:
		...
		/// </summary>
		RecipeBuildLocationManager(const std::map<std::string, KnownLanguage> &knownLanguageLookup)
		...
		Path GetOutputDirectory(
			...
			if (RootRecipeExtensions::TryFindRootRecipeFile(packageRoot, rootRecipeFile)) {
				...
				if (!recipeCache.TryGetRootRecipe(rootRecipeFile, rootRecipe)) {
					...
					Log::Error(
						"Failed to load the root recipe file: {}", rootRecipeFile.ToString());
				...
				if (rootRecipe->HasOutputRoot()) {
					...
					if (knownLanguageResult == _knownLanguageLookup.end()) {
						throw std::runtime_error(std::format("Unknown language: {}", language));
					}

					rootOutput =
						rootOutput +
						Path(std::format("./{}/", knownLanguageResult->second.ExtensionName));
					...
					rootOutput =
						...
						Path(
							std::format(
								"./{}/{}/", name.GetName(), recipe.GetVersion().ToString()));
