// <copyright file="dependency-target-set.cpp" company="Soup">
...
namespace Soup::Core {
	/// <summary>
	/// The dependency target set used to track all directories and macros in
	/// use by a package build runtime
	/// </summary>
	class DependencyTargetSet {
	public:
		// The active set of macros that are only used to ensure the subgraph
		// macros do not collide with package versions in the runtime closure
		std::map<std::string, std::string> GenerateSubGraphMacros;

		// The active set of macros that are only used by this package during
		// generate
		std::map<std::string, std::string> GenerateCurrentMacros;

		// The active set of directories that can accessed during generate by
		// this package only
		...
		// The recursive set of macros that all upstream dependencies can
		// utilize during evaluate
		std::map<std::string, std::string> EvaluateRecursiveMacros;

		// The active set of directories that can accessed during evaluate by
		// this package only
