...
export module Soup.Core.Tests:OperationGraphTests;
...
namespace Soup::Core::UnitTests {
	...
	public:
		...
		void GetOperationInfo_MissingThrows() {
			auto uut = OperationGraph(std::vector<OperationId>(), std::vector<OperationInfo>());

			auto exception = Assert::Throws<std::runtime_error>(
				[&uut]() { auto operationInfo = uut.GetOperationInfo(1); });
		...
		void AddOperation() {
			...
			uut.AddOperation(OperationInfo(
				1,
				"TestOperation",
				CommandInfo(Path("C:/Root/"), Path("./DoStuff.exe"), {"arg1", "arg2"}),
				{
