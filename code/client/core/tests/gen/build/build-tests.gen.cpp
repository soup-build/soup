...
export TestState RunBuildTests() {
	...
	state += Soup::Test::RunTest(className, "Execute_NoDependencies_UpToDate", [&testClass]() {
		testClass->Execute_NoDependencies_UpToDate();
	});
	state +=
	...
	state += Soup::Test::RunTest(
		className, "Execute_NoDependencies_HasPreprocessors_UpToDate", [&testClass]() {
			testClass->Execute_NoDependencies_HasPreprocessors_UpToDate();
		});
