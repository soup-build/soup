...
export TestState RunRecipeBuildLocationManagerTests() {
	...
	TestState state = {0, 0};
	state += Soup::Test::RunTest(className, "GetOutputDirectory_Simple", [&testClass]() {
		testClass->GetOutputDirectory_Simple();
	});
	state += Soup::Test::RunTest(className, "GetOutputDirectory_RootRecipe", [&testClass]() {
		testClass->GetOutputDirectory_RootRecipe();
	});
