...
namespace Soup::Core::UnitTests {
	...
	public:
		// [[Fact]]
		void Initialize() {
			auto uut = PackageName("user1", "my-package");
		...
		void OperatorEqual() {
			...
			Assert::AreEqual(PackageName("user1", "my-package"), uut, "Verify are equal.");
		}
		...
		void OperatorNotEqualOwner() {
			...
			Assert::AreNotEqual(PackageName("user2", "my-package"), uut, "Verify are not equal.");
		}
		...
		void OperatorNotEqualName() {
			...
			Assert::AreNotEqual(PackageName("user1", "my-package2"), uut, "Verify are not equal.");
		}
		...
		void ParseValues(std::string value, std::optional<std::string> owner, std::string name) {
			...
			Assert::AreEqual(PackageName(owner, name), uut, "Verify matches expected values.");
		}
		...
		// [[InlineData("user1", "name", "user1|name")]]
		void ToStringValues(
			std::optional<std::string> owner, std::string name, std::string expected) {
