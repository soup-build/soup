...
namespace Soup::Core::UnitTests {
	...
	public:
		...
		void InitializeNamed() {
			auto uut = PackageReference("C#", "user1", "my-package", SemanticVersion(1, 2, 3));
			...
			Assert::AreEqual(
				SemanticVersion(1, 2, 3), uut.GetVersion(), "Verify version matches expected.");
		}
		...
		void OperatorEqualNameVersion() {
			...
			Assert::AreEqual(
				PackageReference("C#", "user1", "my-package", SemanticVersion(1, 2, 3)),
				uut,
				"Verify are equal.");
		}
		...
		void OperatorEqualPath() {
			...
			Assert::AreEqual(PackageReference(Path("../my-package")), uut, "Verify are equal.");
		}
		...
		void OperatorNotEqualOwner() {
			...
			Assert::AreNotEqual(
				PackageReference("C#", "user2", "my-package", SemanticVersion(1, 2, 3)),
				uut,
				"Verify are not equal.");
		}
		...
		void OperatorNotEqualName() {
			...
			Assert::AreNotEqual(
				PackageReference("C#", "user1", "my-package2", SemanticVersion(1, 2, 3)),
				uut,
				"Verify are not equal.");
		}
		...
		void OperatorNotEqualVersion() {
			...
			Assert::AreNotEqual(
				PackageReference("C#", "user1", "my-package", SemanticVersion(11, 2, 3)),
				uut,
				"Verify are not equal.");
		}
		...
		void OperatorNotEqualPath() {
			...
			Assert::AreNotEqual(
				PackageReference(Path("../my-package2")), uut, "Verify are not equal.");
		}

		// [[Theory]]
		// [[InlineData("name", std::nullopt, std::nullopt, "name", std::nullopt)]]
		// [[InlineData("name@1.2.3", std::nullopt, std::nullopt, "name", Soup::SemanticVersion(1,
		// 2, 3))]]
		// [[InlineData("[C#]name", "C#", std::nullopt, "name", std::nullopt)]]
		// [[InlineData("[C#]name@1.2.3", "C#", std::nullopt, "name", Soup::SemanticVersion(1, 2,
		// 3))]]
		// [[InlineData("user1|name", std::nullopt, "user1", "name", std::nullopt)]]
		// [[InlineData("user1|name@1.2.3", std::nullopt, "user1", "name", Soup::SemanticVersion(1,
		// 2, 3))]]
		// [[InlineData("[C#]user1|name", "C#", "user1", "name", std::nullopt)]]
		// [[InlineData("[C#]user1|name@1.2.3", "C#", "user1", "name", Soup::SemanticVersion(1, 2,
		// 3))]]
		void ParseNamedValues(
			std::string value,
		...
		void ParsePathValues(std::string value) {
			auto uut = PackageReference::Parse(value);
			Assert::AreEqual(PackageReference(Path(value)), uut, "Verify matches expected values.");
		}
		...
		// [[InlineData(std::nullopt, std::nullopt, "name", Soup::SemanticVersion(1, 2, 3),
		// "name@1.2.3")]]
		// [[InlineData("C#", std::nullopt, "name", std::nullopt, "[C#]name")]]
		// [[InlineData("C#", std::nullopt, "name", Soup::SemanticVersion(1, 2, 3),
		// "[C#]name@1.2.3")]]
		// [[InlineData(std::nullopt, "user1", "name", std::nullopt, "user1|name")]]
		// [[InlineData(std::nullopt, "user1", "name", Soup::SemanticVersion(1, 2, 3),
		// "user1|name@1.2.3")]]
		// [[InlineData("C#", "user1", "name", std::nullopt, "[C#]user1|name")]]
		// [[InlineData("C#", "user1", "name", Soup::SemanticVersion(1, 2, 3),
		// "[C#]user1|name@1.2.3")]]
		void ToStringNamedValues(
			std::optional<std::string> language,
			std::optional<std::string> owner,
			std::string name,
			std::optional<SemanticVersion> version,
		...
		void ToStringPathValues(std::string path) {
			...
			auto value = uut.ToString();
			Assert::AreEqual(path, value, "Verify matches expected value.");
		}
	};
}
