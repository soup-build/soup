...
export module Soup.Core:RecipeTests;
...
namespace Soup::Core::UnitTests {
	...
	public:
		...
		void InitializerDefault() {
			...
			Assert::AreEqual(
				std::vector<std::string>(), uut.GetDependencyTypes(), "Verify no dependency types");
		}
		...
		void InitializerAll() {
			...
			Assert::AreEqual<std::string_view>(
				"my-package", uut.GetName(), "Verify name matches expected.");
			...
			Assert::AreEqual(
				SemanticVersion(1, 2, 3), uut.GetVersion(), "Verify version is correct.");
