...
namespace Soup::Core::Generate
{
	...
	private:
		...
		static inline const char* soupTestModuleSource =
			...
			"	static operationProxies { __operationProxies }\n"
			"	static logs { __logs }\n"
			"\n"
			"	static globalState {\n"
			...
			"		if (__operationProxies is Null) Fiber.abort(\"Operation Proxies not initialized.\")\n"
			"		__operationProxies.add(SoupTestOperationProxy.new(title, executable, arguments, workingDirectory, declaredInput, resultFile, finalizerTask))\n"
			"	}\n"
			"\n"
			"	static info(message) {\n"
		...
		static inline const char* soupModuleSource =
			...
			"		if (!(workingDirectory is String)) Fiber.abort(\"WorkingDirectory must be a string.\")\n"
			"		if (!(declaredInput is List)) Fiber.abort(\"DeclaredInput must be a list.\")\n"
			"		if (!(resultFile is String)) Fiber.abort(\"Result File Task must be a string.\")\n"
			"		if (!(finalizerTask is String)) Fiber.abort(\"Finalizer Task must be a string.\")\n"
			"		SoupTest.createOperationProxy(title, executable, arguments, workingDirectory, declaredInput, resultFile, finalizerTask)\n"
			"	}\n"
			"\n"
			"	static info(message) {\n"
