...
namespace Soup::Core::Generate
{
	...
	private:
		...
		static inline const char* soupTestModuleSource =
			...
			"		__operationProxies.add(SoupTestOperationProxy.new(title, executable, arguments, workingDirectory, declaredInput, declaredOutput, finalizerTask))\n"
			"	}\n"
			"\n"
			"	static info(message) {\n"
		...
		static inline const char* soupModuleSource =
			...
			"		SoupTest.createOperationProxy(title, executable, arguments, workingDirectory, declaredInput, declaredOutput, finalizerTask)\n"
			"	}\n"
			"\n"
			"	static info(message) {\n"
